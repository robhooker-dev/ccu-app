...
const T = {
  ...
  es: {
    ...
    descPlaceholder: "Proporcione tantos detalles como sea posible.",
...
const SEED_REPORTS = [
  {
    ...
    content: "A senior officer in my unit has been accepting cash payments from a local business owner in exchange for not pursuing enforcement action. I have witnessed this personally on three separate occasions over the past two months.",
    riskLevel: "HIGH", urgency: "Within 24h",
    riskReason: "Repeated financial corruption involving named officer. Direct witness evidence.",
    ...
    broadcastOptIn: true, language: "en", files: [],
  },
  {
    id: "RPT-3341-BVMW", token: "RPT-3341-BVMW",
    timestamp: new Date(Date.now() - 2 * 86400000), category: "Unauthorised Accessing",
    content: "Evidence has been removed from the custody suite. A bag of items from a drugs arrest went missing before it could be booked in.",
    riskLevel: "CRITICAL", urgency: "Immediate",
    riskReason: "Active evidence tampering in custody suite. Immediate intervention required.",
    keywords: ["evidence", "custody", "tampering"],
    status: "Escalated",
    messages: [], broadcastOptIn: false, language: "en", files: [],
  },
  {
    id: "RPT-9912-LRDZ", token: "RPT-9912-LRDZ",
    timestamp: new Date(Date.now() - 14 * 86400000), category: "Unauthorised Accessing",
    content: "An officer in my team has been running PNC and intelligence checks on a former partner. The searches were not linked to any live investigation.",
    riskLevel: "MEDIUM", urgency: "Within 48h",
    riskReason: "Unlawful access of police systems for personal purposes.",
    keywords: ["PNC misuse", "DPA"],
    status: "Closed",
    messages: [
      { from: "CCU", content: "This report has been reviewed. An internal investigation has been opened under reference DPS-7712.", timestamp: new Date(Date.now() - 10 * 86400000), id: "m2" },
      { from: "Reporter", content: "Thank you. Will I receive updates on the outcome?", timestamp: new Date(Date.now() - 9 * 86400000), id: "m3" },
      { from: "CCU", content: "We will provide updates where possible. Your anonymity remains fully protected.", timestamp: new Date(Date.now() - 8 * 86400000), id: "m4" },
    ],
    broadcastOptIn: true, language: "en", files: [],
  },
  {
    ...
    content: "I have information that an officer may be leaking operational intelligence to an organised crime group.",
    riskLevel: "CRITICAL", urgency: "Immediate",
    riskReason: "Intelligence leakage to OCG poses immediate threat to officer safety.",
    keywords: ["OCG", "intelligence leak"],
    status: "New",
    messages: [], broadcastOptIn: false, language: "en", files: [],
  },
  {
    ...
    content: "I have noticed a pattern of inflated expense claims in my department. Several officers appear to be claiming overtime not worked.",
    riskLevel: "MEDIUM", urgency: "Within 48h",
    riskReason: "Systematic financial fraud involving public funds.",
    keywords: ["fraud", "expenses"],
    status: "Under Review",
    messages: [], broadcastOptIn: true, language: "en", files: [],
  },
];

const SEED_BROADCASTS = [
  { id: "bc1", timestamp: new Date(Date.now() - 5 * 86400000), content: "The CCU has expanded its review capacity. All reports are now being assessed within 24 hours." },
];
...
const RiskBadge = ({ level, small }) => {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: small ? "2px 7px" : "3px 10px", borderRadius: 4, background: c.bg, border: `1px solid ${c.border}`, color: c.color, fontFamily: "'Calibri',sans-serif", fontSize: small ? 9 : 10, fontWeight: 700, letterSpacing: "0.06em", whiteSpace: "nowrap" }}>
      {c.dot} {c.label}
    </span>
  );
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 9px", borderRadius: 4, background: c.bg, border: `1px solid ${c.border}`, color: c.color, fontFamily: "'Calibri',sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", whiteSpace: "nowrap" }}>
      {status}
    </span>
  );
};
  ...
  return (
    <div className="fg">
      ...
      <div
        ...
        <div className="dropzone-s">PDF, Word, Images, Video, Audio · Max {MAX_SIZE_MB}MB · Up to 5 files</div>
...
.logo{display:flex;align-items:center;gap:9px;cursor:pointer;
  font-family:'Calibri',sans-serif;font-size:17px;
  font-weight:700;letter-spacing:.12em;color:var(--t1)}
.logo-ic{width:26px;height:26px;display:flex;align-items:center;justify-content:center}
.sec-tag{font-size:8px;font-weight:600;letter-spacing:.08em;color:var(--gr);
  background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.25);
  padding:2px 6px;border-radius:3px}
.ver{font-size:9px;color:var(--t3)}
...
.token-v{font-size:28px;font-weight:700;color:var(--pr);letter-spacing:.14em;margin-bottom:8px}
...
.stat-v{font-size:38px;font-weight:700;line-height:1;margin-bottom:3px}
...
.rid{font-size:10px;font-weight:600;color:var(--t2);white-space:nowrap}
...
.rtime{font-size:10px;color:var(--t3);white-space:nowrap}
...
.msgs::-webkit-scrollbar{width:3px}.msgs::-webkit-scrollbar-thumb{background:var(--b1);border-radius:2px}
...
.mm{font-size:9px;color:var(--t3);margin-top:4px}
...
.bc-t{font-size:9px;color:var(--t3)}
...
.sec{font-size:14px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;
  color:var(--t2);margin-bottom:14px;display:flex;align-items:center;gap:7px}
.tag{display:inline-block;padding:2px 7px;border-radius:3px;font-size:9px;font-weight:600;
  background:rgba(91,127,255,.1);border:1px solid rgba(91,127,255,.22);color:#818cf8;
  letter-spacing:.04em;margin:2px}
.ccu-b{font-size:8px;font-weight:700;letter-spacing:.1em;padding:2px 7px;border-radius:3px;
...
.cbr input{margin-top:2px;accent-color:var(--pr)}
...
.file-size{font-size:10px;color:var(--t3)}
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are a risk assessment AI for a police counter-corruption unit. Analyse the report and respond ONLY with valid JSON:
{"riskLevel":"CRITICAL|HIGH|MEDIUM|LOW","riskReason":"One concise sentence max 20 words","urgency":"Immediate|Within 24h|Within 48h|Routine","keywords":["keyword1","keyword2","keyword3"]}`,
      ...
      if (l.includes("weapon") || l.includes("organised crime") || l.includes("intelligence"))
      ...
      if (l.includes("fraud") || l.includes("misconduct"))
  ...
  const handleSubmit = async () => {
    ...
    setNewTok(token);
    setDraft({ category: "", content: "", optIn: false, files: [] });
    setSubmitting(false);
    setView("success");
  };
  ...
  const LB = () => (
    <div className="lb">
      {LANGUAGES.map(l => (
        <button key={l.code} className={`lng ${lang === l.code ? "on" : ""}`} onClick={() => setLang(l.code)} title={l.full}>
          {l.label}
        </button>
      ))}
    </div>
    ...
    <div className="app"><style>{CSS}</style>
      ...
      <main className="main">
        <div className="page" style={{ maxWidth: 680, textAlign: "center" }}>
          <LB />
          <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: "clamp(32px,5vw,52px)", fontWeight: 800, letterSpacing: ".1em", marginBottom: 10 }}>{t.appName}</div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 16, maxWidth: 520, margin: "0 auto 24px" }}>
            <div onClick={() => setView("submit")} style={{ background: "var(--s1)", border: "1px solid var(--b1)", borderRadius: 12, padding: "26px 22px", cursor: "pointer", textAlign: "left", transition: "all .2s", minWidth: 200 }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--pr)"; e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 28px rgba(91,127,255,0.15)"; }}
              ...
              <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 19, fontWeight: 700, marginBottom: 5 }}>{t.reporterTitle}</div>
              <div style={{ fontSize: 12, color: "var(--t2)" }}>{t.reporterSub}</div>
            </div>
          </div>
    ...
    <div className="app"><style>{CSS}</style>
      ...
      <main className="main">
        <div className="page">
          ...
          <LB />
          <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 26, fontWeight: 800, marginBottom: 22 }}>🛡️ {t.submitReport}</div>
          ...
          <div className="card">
            ...
            <div className="fg">
              ...
              <div style={{ fontSize: 10, color: draft.content.length < 20 ? "var(--am)" : "var(--t3)", marginTop: 5 }}>
          ...
          {draftMsg && <div style={{ color: "var(--gr)", fontSize: 12, marginBottom: 12 }}>✓ {draftMsg}</div>}
      ...
      <div className="app"><style>{CSS}</style>
        ...
        <main className="main">
          <div className="page" style={{ maxWidth: 560, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 28, fontWeight: 800, marginBottom: 8 }}>Report Received</div>
            ...
            {r && (
              <div style={{ background: RISK_CONFIG[r.riskLevel]?.bg, border: `1px solid ${RISK_CONFIG[r.riskLevel]?.border}`, borderRadius: 10, padding: "16px 20px", marginBottom: 22, textAlign: "left" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 9, flexWrap: "wrap" }}>
                  ...
                  {r.urgency && <span style={{ fontSize: 10, color: "var(--t2)" }}>⏱ {r.urgency}</span>}
    ...
    <div className="app"><style>{CSS}</style>
      ...
      <main className="main">
        <div className="page" style={{ maxWidth: 460 }}>
          <button className="back" onClick={() => setView("landing")}>← {t.back}</button>
          <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 24, fontWeight: 800, marginBottom: 6 }}>📬 Anonymous Inbox</div>
          <div style={{ fontSize: 13, color: "var(--t2)", marginBottom: 26 }}>{t.enterToken}</div>
          <div className="card">
            <div className="fg">
              <label className="lbl">{t.tokenLabel}</label>
              <input className="inp" placeholder="RPT-XXXX-XXXX" value={tokIn} onChange={e => { setTokIn(e.target.value); setTokErr(false); }} style={{ fontSize: 16, letterSpacing: ".1em" }} onKeyDown={e => e.key === "Enter" && handleInbox()} />
          ...
          <div style={{ fontSize: 11, color: "var(--t3)", textAlign: "center", marginTop: 14 }}>
            Demo token: <span style={{ color: "var(--t2)" }}>RPT-7823-XKQP</span>
          </div>
        </div>
      </main>
    </div>
      ...
      <div className="app"><style>{CSS}</style>
        ...
        <main className="main">
          <div className="page" style={{ maxWidth: 620 }}>
            ...
            <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 4 }}>📬 Your Inbox</div>
            <div style={{ fontSize: 11, color: "var(--t2)", marginBottom: 22, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
      ...
      <main className="main">
        <div className="page" style={{ maxWidth: 400 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 26 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 21, fontWeight: 800 }}>{t.ccuLoginTitle}</div>
          ...
          <div className="card">
            <div className="fg">
              ...
              <input className="inp" type="password" placeholder="Enter access code" value={pw} onChange={e => { setPw(e.target.value); setPwErr(false); }} style={{ letterSpacing: ".12em" }} onKeyDown={e => e.key === "Enter" && handleLogin()} />
            ...
            <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 11 }}>Demo: CCU2024</div>
          </div>
        </div>
      </main>
    </div>
      ...
      <main className="main">
        <div className="page-w">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 24, fontWeight: 800, marginBottom: 4 }}>🔒 {t.dashboard}</div>
              <div style={{ fontSize: 11, color: "var(--t2)", display: "flex", alignItems: "center" }}>
          ...
          <div className="dash-g">
            <div>
              <div className="fbar">
                {["ALL", "CRITICAL", "HIGH", "MEDIUM", "LOW"].map(f => <button key={f} className={`fb ${filter === f ? "on" : ""}`} onClick={() => setFilter(f)}>{f}</button>)}
              </div>
              <div className="rl sc">
                {filtered.length === 0
        ...
        <main className="main">
          <div className="page" style={{ maxWidth: 740 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
              <div>
                <div style={{ fontSize: 12, color: "var(--t2)", marginBottom: 5 }}>{report.id}</div>
                <div style={{ fontFamily: "'Calibri',sans-serif", fontSize: 22, fontWeight: 800 }}>{report.category}</div>
              </div>
            ...
            <div className="dg">
              ...
              <div className="di"><div className="dk">Language</div><div className="dv">{LANGUAGES.find(l => l.code === report.language)?.full || "English"}</div></div>
            ...
            <div style={{ background: RISK_CONFIG[report.riskLevel]?.bg, border: `1px solid ${RISK_CONFIG[report.riskLevel]?.border}`, borderRadius: 10, padding: "15px 18px", marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 9, flexWrap: "wrap" }}>
                ...
                {report.urgency && <span style={{ fontSize: 10, color: "var(--t2)" }}>⏱ {report.urgency}</span>}
